...
exports.getLocationInfo = functions.https.onRequest(
  async (request, response) => {
    cors(async (request, response) => {
      ...
      var options = {
        url:
          "http://open.mapquestapi.com/geocoding/v1/reverse?key=qzttnJ2q62vLFGf8spd0LJDkY3ksgwa9&location=" +
          lat +
          "," +
          lng,
        json: true,
      ...
      await admin
        ...
        .once("value", async (snapshot) => {
          ...
          await admin
            .database()
            .ref("counties/" + snapshot.val().county)
            .once("value", async (snapshot) => {
              var query = admin
                .firestore()
                .collection("bus_stop")
              ...
              query.get().then(async (querySnapshot) => {
                await querySnapshot.forEach((documentSnapshot) => {
                  ...
                  distance = Math.sqrt(
                    Math.pow(tmp.lat - lat, 2) + Math.pow(tmp.lng - lng, 2)
                  );
                ...
                return response.status(200).json({
                  data: {
                    closest_stop_distance_miles: cur_distance * 69,
                    zipcode: zip,
                    countyInfo: snapshot.val(),
                    zipInfo: zipData,
                    bus_stop: closest_stop,
                  },
                });
              });
            });
...
exports.androidEndpoint = functions.https.onRequest( async (request, response) => 
{
	var lat = request.body.data['lat'];
	var lng = request.body.data['lng'];
